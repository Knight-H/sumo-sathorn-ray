...
_temp_episode = None  # since episode is not passed to on_result
...
##    print("this is epi length", episode.length)
    ...
    episode.user_data["backlog"].append(episode.last_info_for()['backlog'])
    for jam_length in _JAM_LENGTHS:
...
def on_train_result(info):
    ...
    print("Episode User Data on end ", episode.user_data)
    ...
    _result["actions"] = episode.user_data["action"]
    with open(os.path.join(_DIR_TO_WRITE, 'ep{}_actions.pickle'.format(EPISODE_NO)), 'wb') as f:
        pickle.dump(episode.user_data["action"], f, pickle.HIGHEST_PROTOCOL)

    _phase_dict = _calculate_avg_green_time(episode.user_data["action"])
    ...
    # --- Travel Time ---
    for travel_time in _TRAVEL_TIMES:
        _result[travel_time] = np.mean(episode.user_data[travel_time], dtype=np.float32)
        with open(os.path.join(_DIR_TO_WRITE, 'ep{}_{}.pickle'.format(EPISODE_NO, travel_time)), 'wb') as f:
            pickle.dump(episode.user_data[travel_time], f, pickle.HIGHEST_PROTOCOL)

    # --- Total Throughput ---                                 
    _result["total_throughput"] = np.mean(episode.user_data["total_throughput"], dtype=np.float32)
    with open(os.path.join(_DIR_TO_WRITE, 'ep{}_total_throughput.pickle'.format(EPISODE_NO)), 'wb') as f:
        pickle.dump(episode.user_data["total_throughput"], f, pickle.HIGHEST_PROTOCOL)

    # --- Backlog --- 
    _result["backlog"] = np.mean(episode.user_data["backlog"], dtype=np.float32)
    with open(os.path.join(_DIR_TO_WRITE, 'ep{}_total_throughput.pickle'.format(EPISODE_NO)), 'wb') as f:
        pickle.dump(episode.user_data["backlog"], f, pickle.HIGHEST_PROTOCOL)

    # --- Jam Length ---
    for jam_length in _JAM_LENGTHS:
        _result[jam_length] = np.mean(episode.user_data[jam_length], dtype=np.float32)
        with open(os.path.join(_DIR_TO_WRITE, 'ep{}_{}.pickle'.format(EPISODE_NO, jam_length)), 'wb') as f:
            pickle.dump(episode.user_data[jam_length], f, pickle.HIGHEST_PROTOCOL)
